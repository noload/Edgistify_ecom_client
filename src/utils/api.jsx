...
export const registerUser =  async(email,password,fullName)=>{
    try {
        const resp = await axios.post(`${API_URL}/api/user/register`,{email,password,fullName});
        return resp.data;
    } catch (error) {
...
export const fetchProductsApi = async(category,token)=>{
  ...
  if (!token) {
    ...
    const response = await axios.get(`${API_URL}/api/product/${category}`,{headers:{Authorization:`Bearer ${token}`}})
